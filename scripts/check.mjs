...
for(const file of [...files,...await walk(resolve(root,'bridge'))].filter(f=>/\.(m?js)$/.test(f))){const r=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(r.status!==0)throw new Error(r.stderr);const content=await readFile(file,'utf8');for(const m of content.matchAll(/from\s+['"](\.[^'"]+)['"]/g))await stat(resolve(dirname(file),m[1]));}
...
for(const m of html.matchAll(/(?:src|href)="(\.\/[^"#]+)"/g))await stat(resolve(root,'dist',m[1]));
