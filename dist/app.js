...
import {Bridge} from './modules/bridge-client.js?v=20260905-sacn';
...
let selected={strip:project.strips[0].id,index:0},selection=new Set(),advanced=false,view='map',labels=false,placing=false,bridge=null,bridgeOutput=null,camera=new Camera($('cameraVideo')),photoURL=null,busy=false,scanning=false,paused=false,scanAbort=null,scanDone=0,scanTotal=0,editId=null,scanElapsed=0,scanStarted=0,sessionConfigured=false,photoRatio=16/9;
...
function updateProtocolHint(){$('protocolHint').textContent=$('controllerProtocol').value==='sacn'?'StripR universe 0 maps to sACN universe 1 · unicast UDP 5568. In WLED, select E1.31 and Multiple RGB (or RGBW).':'StripR universe 0 maps to Art-Net universe 0 · unicast UDP 6454.';}
...
function renderControls(){
  ...
  for(const id of ['addStrips','cameraButton','startCameraEmpty','cameraSelect','photoButton','connectButton','bridgeUrl','bridgeToken','controllerProtocol','controllerTarget','openProject','projectName','demoButton','helpDemo','brightness','savePixel','placePixel','pixelX','pixelY','simpleMode','advancedMode','quickQuantity','quickCount','applyQuick','smartLines','selectStrip','selectAll','transformX','transformY','transformScale','transformRotation','applyTransform'])$(id).disabled=locked;
  for(const id of ['bridgeUrl','bridgeToken','controllerProtocol','controllerTarget'])$(id).disabled=locked||Boolean(bridge);
  ...
  $('bridgeStatus').textContent=bridge?(bridgeOutput?.protocol==='sacn'?'sACN':bridgeOutput?.protocol==='artnet'?'ART-NET':'CONNECTED'):project.demo?'DEMO':'OFFLINE';$('bridgeStatus').className='status '+(bridge?'online':project.demo?'demo':'');
...
async function connectBridge(silent=false){if(busy||scanning)return;busy=true;renderControls();try{if(bridge){await bridge.blackout();bridge=null;bridgeOutput=null;sessionConfigured=false;if(!silent)notify('Controller disconnected.');}else{const candidate=new Bridge($('bridgeUrl').value.trim(),$('bridgeToken').value.trim()),health=await candidate.health(),managed=health.output&&health.adapter!=='demo';if(health.output&&(silent||!$('controllerTarget').value.trim())){$('controllerProtocol').value=health.output.protocol==='sacn'?'sacn':'artnet';$('controllerTarget').value=health.output.target||'';updateProtocolHint();}if(silent&&managed&&!health.output.configured)return;if(managed&&!silent){const protocol=$('controllerProtocol').value,target=$('controllerTarget').value.trim();if(!target)throw new Error('Enter the Art-Net or sACN controller address.');await candidate.output(protocol,target);bridgeOutput={protocol,target,configured:true,port:protocol==='sacn'?5568:6454,universeOffset:protocol==='sacn'?1:0};}else bridgeOutput=health.output||{protocol:health.adapter||'unknown',configured:true};await candidate.configure(project.strips);bridge=candidate;sessionConfigured=true;if(project.demo&&health.adapter!=='demo'){const before=snapshot();project.demo=false;project.strips.forEach(s=>s.points.fill(null));recordChange(before,'leave demo map');selectOnly(project.strips[0].id,0,false);}if(!silent)notify(`Connected to ${health.name||'StripR app'}${health.adapter==='demo'?' — server is in simulation mode; no hardware output':bridgeOutput?.target?` · ${bridgeOutput.protocol==='sacn'?'sACN':'Art-Net'} → ${bridgeOutput.target}`:''}.`);save();scanTotal=0;}}catch(e){bridgeOutput=null;if(!silent)notify(e.message,true);}finally{busy=false;render();}}
$('connectButton').onclick=()=>connectBridge();
$('controllerProtocol').onchange=updateProtocolHint;updateProtocolHint();
