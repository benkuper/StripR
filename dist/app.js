import {COLORS,DEFAULT_SETTINGS,newStrip,newProject,validateProject,validatePatch,integer,pixelAddress,totals,serializeProject,demoPosition} from './modules/model.js';
...
import {Bridge} from './modules/bridge-client.js';
...
function renderInspector(){const s=currentStrip(),p=s.points[selected.index],a=pixelAddress(s,selected.index);$('inspectPixel').max=s.count;$('inspectPixel').value=selected.index+1;$('pixelState').textContent=p?`${p.source==='manual'?'Manually placed':p.source==='demo'?'Demo coordinate':p.source==='interpolated'?'Inferred from straight strip':'Detected'} · ${Math.round(p.confidence*100)}% confidence`:'Not mapped';$('pixelX').value=p?p.x.toFixed(4):'';$('pixelY').value=p?p.y.toFixed(4):'';$('pixelAddress').textContent=`U${a.universe} · CH ${a.channel}–${a.channel+2} · ${s.order.toUpperCase()}`;$('previousPixel').disabled=selected.index===0;$('nextPixel').disabled=selected.index===s.count-1;$('clearPixel').disabled=!p||scanning||busy;$('identifyPixel').disabled=(!bridge&&!project.demo)||scanning||busy;}
...
function openStrips(id){editId=id||null;const s=id?project.strips.find(s=>s.id===id):null;const next=Math.max(...project.strips.map(s=>pixelAddress(s,s.count-1).universe))+1;$('stripDialogTitle').textContent=s?'Edit strip':'Add strips';$('bulkHint').textContent=s?'Adjust this strip’s name, length, or controller patch.':'Same LED count? Add them all in one go.';$('quantityLabel').hidden=!!s;$('stripQuantity').value=1;$('stripLeds').value=s?.count||60;$('stripName').value=s?.name||'Strip';$('stripUniverse').value=s?.universe??Math.min(255,next);$('stripChannel').value=s?.channel||1;$('stripOrder').value=s?.order||'rgb';$('removeStrip').hidden=!s||project.strips.length===1;$('saveStrips').textContent=s?'Save strip':'Add strips';$('stripError').textContent='';$('stripPatch').hidden=!advanced;$('stripDialog').showModal();}
$('addStrips').onclick=()=>openStrips();
$('stripForm').onsubmit=async e=>{e.preventDefault();try{const quantity=integer($('stripQuantity').value,1,128,'Number of strips'),count=integer($('stripLeds').value,1,4096,'LED count'),universe=integer($('stripUniverse').value,0,255,'Universe'),channel=integer($('stripChannel').value,1,510,'Start channel'),name=$('stripName').value.trim();if(!name)throw new Error('Enter a strip name.');let strips=project.strips.map(s=>({...s,points:[...s.points]}));if(editId){const s=strips.find(s=>s.id===editId);if(s.count>count&&s.points.slice(count).some(Boolean)&&!await confirmAction('Shorten this strip?','Coordinates beyond the new LED count will be removed. Export JSON first if you need a backup.'))return;s.points=Array.from({length:count},(_,i)=>s.points[i]||null);Object.assign(s,{count,name,universe,channel,order:$('stripOrder').value});}else{let u=universe;for(let i=0;i<quantity;i++){const s=newStrip(strips.length,count,u);Object.assign(s,{name:quantity===1?name:`${name} ${String(i+1).padStart(2,'0')}`,channel,order:$('stripOrder').value});strips.push(s);u=pixelAddress(s,count-1).universe+1;}}validatePatch(strips);project.strips=strips;$('stripDialog').close();patchChanged();}catch(e){$('stripError').textContent=e.message;}};
